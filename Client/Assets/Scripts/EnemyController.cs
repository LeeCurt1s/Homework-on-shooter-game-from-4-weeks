...
public class EnemyController : MonoBehaviour
{
    ...
    private float AverageInterval //добавили свойсвто 
    {
        get
        {
            int reciveTimeIntervalCount = _receiveTimeInterval.Count;
            float summ = 0;
            for (int i = 0; i < reciveTimeIntervalCount; i++)
            {
                summ += _receiveTimeInterval[i];
            }
            return summ / reciveTimeIntervalCount;
        }
    }
